...
import DraggableText from './utils/Draggable-Text-Popup';
...
function Header() {
  ...
  return (
    <SimpleGrid columns={[1,1,2]} row={[2,2,1]} h='fit-content' w='100%' mx='auto' my={10} spacing={[10 ,5,0]} alignItems='center' justifyContent={'center'}>
      <GridItem position='relative' h='100%' w={['95%','80%','80%']} m='auto' alignItems='center' display='flex' justifyContent='center'>
            <Image objectFit='cover' h='100%' w='100%' src='/pages/Home/Workspace.jpeg' alt='My WorkSpace' />
        <DraggableText/>
      </GridItem>
      <AnimatePresence initial={true}>
        <GridItem h='fit-content' mx='auto' my={['0','0','auto']} >
            <VStack spacing={[4,6]} my='auto' mx={['auto','auto','0']} w={['95%','80%','60%']} h='100%' alignItems='flex-start'>
                ...
                <motion.div initial={{transform: 'translateY(-3rem)'}} transition={{duration: 0.4}} animate={{transform: 'translateY(0rem)'}} exit={{transform: 'translateY(0rem)'}}>
                  <Button onClick={() => Router.push('/about')} _focus={{textDecoration:'none'}} role='group' variant='outline' borderBottom='.2rem solid teal' transition='all .2s' _hover={{backgroundPosition: "100%",color:'white'}} backgroundSize='230%' bgImage={buttonBg}>
