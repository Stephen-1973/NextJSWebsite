...
import { HStack, Box, IconButton,Heading, Text, Flex, Image } from '@chakra-ui/react';
...
function NavBar() {
  ...
  const HamburgerIconBox = useColorModeValue('black','white')
  return (
    <AnimatePresence>
      <motion.div initial={{y:-20,opacity:0}} animate={{y:0,opacity: 1}} transition={{duration: 0.2}}>
        <Box w='full' alignItems='center' display='flex' justifyContent={['space-evenly','space-evenly']}>
          <Flex w='50%' justifyContent='space-evenly' alignItems='center'>
            <HStack alignItems='center' justifyContent='space-evenly' spacing={2}>
            ...
            <Text display={['none','none','auto']} fontSize='110%'>
              Work
            </Text>
            <Box display={['none','none','auto']} cursor='pointer' fontSize='110%'>
              <HStack>
