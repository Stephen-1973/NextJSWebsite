...
function Header() {
  const buttonBg = useColorModeValue("linear-gradient(120deg, transparent 0%, #fff 50%, teal 50%)", 'linear-gradient(120deg, transparent 0%, black 50%, teal 50%)')
  
  return (
    <SimpleGrid columns={[1,1,2]} rows={[2,2,1]} flexDirection='row' h={['70vh','65vh','50vh']} w='100%' mx='auto' mb={0} mt={10}>
        ...
        <AnimatePresence initial={true}>
          <VStack spacing={[4,6]} my='auto' mx={['auto','auto','0']} w={['80%','80%','60%']} h='80%' transform='translateY(-2.5rem)' alignItems='flex-start'>
              <motion.div animate={{transform: 'translateY(3rem)'}} transition={{duration: 0.6}} initial={{transform: 'translateY(0rem)'}} exit={{transform: 'translateY(0rem)'}}>
                <Text textAlign='left' fontWeight='bold' textTransform='uppercase\\4'>Hey!</Text>
              </motion.div>
              ...
              <motion.div animate={{transform: 'translateY(3rem)'}} transition={{duration: 0.4}} initial={{transform: 'translateY(0rem)'}} exit={{transform: 'translateY(0rem)'}}>
                <Button variant='outline' borderBottom='.2rem solid teal' transition='all .2s' _hover={{backgroundPosition: "100%",color:'white'}} backgroundSize='230%' bgImage={buttonBg}>
                  Know more &rarr;
                </Button>
              </motion.div>
            </VStack>
